...
class UploadedFile {
  ...
  public static function moveTempFilesDescriptiveNames($filenames, $keywords) {
    ...
    foreach($filenames as $filename) {
      if ($filename) {
        $uploadFilenames[] = self::moveTempFileDescriptiveName($filename, $keywords);
      }
    }
  ...
  public static function moveTempFileDescriptiveName($tempFilename, $keywords) {
    ...
    if ($tempFilename && (file_exists($tempFilePathname))
        ...
        $keywordsString, $targetPath, $tempFilename)
  {
    ...
    if( ! $keywordsString) {
        ...
        if(sizeof($keywords) < $keywordsCount) {
            $keywordsCount = sizeof($keywords);
        }
        shuffle($keywords);
        $keywords = array_slice($keywords, 0, $keywordsCount);
        ...
        $charactersCount = -4;
        // the four last characters are the dot and the extension
