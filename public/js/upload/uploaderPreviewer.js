...
/**
 * Features:
 * - Background hidden iframes used.
 *   -> var imageUrl = $iframeContent.find('.image').text();
 * - Iframe factory. To avoid that an iframe is created multiple times.
 *   -> factoryIframe()
 * - File extension check before uploading.
 *   -> checkImageType()
 * - No garbage. Non used uploaded files deleted on page exit.
 *   -> $(window).unload()
 * - Informative fail upload messages.
 *   -> .errorMessage()
 *
 * Known flaws:
 * - If an image is uploaded while another one is still loading, the second
 *   image is not processed.
 * - If an image is uploaded while another one is still loading but its upload
 *   was canceled, the second image is not processed and the first one is taken.
 *
...
jQuery.uploaderPreviewer = new Object({
    ...
    messages: {
        ...
        removeButtonTitle: 'Remover la imagen'
    }
});
...
//------------------------------------------------------------------------------
/**
 * Upload dinamically an image and display it.
 *
...
jQuery.uploaderPreviewer.handleResult = function($iframe, $previewDiv) {
    ...
    $('body').remove($iframe.empty().unbind('load'));
};
...
function createImageForm(index) {
    ...
    form += '<td class="imageFormFileField">'
          // if the enctype attribute is assigned with jQuery, IE crashes
