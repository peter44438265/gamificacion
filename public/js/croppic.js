	...
	Croppic.prototype = {
		...
		init: function () {
			...
			//that.objH = that.obj.height();
                        ...
                        that.objW = parseInt(that.obj.attr('data'));
			that.objH = parseInt(that.obj.attr('for'));
		...
		createImgUploadControls: function(){
			...
			if(that.options.customUploadButtonId ===''){ cropControlUpload = '<i class="cropControlUpload"></i><span style="float: left; height: 30px; line-height: 20px; padding-top: 5px;">Subir imagen</span>'; }
			...
			var html =    '<div class="cropControls cropControlsUpload" style="position:relative !important;width: 115px;"> ' + cropControlUpload + cropControlRemoveCroppedImage + ' </div>';
            ...
            if (response.status == 'success') {
                ...
                if (response.html == '') {
                    if(response.url ==''){
                        ...
                        if(response.arr != ''){
                            $("#"+response.arr.cid).attr("src",response.url);
                            $("#bannerbig").val(response.arr.bid);
                        }else{
                            /*wizard*/
                            $("#img_perf_wiz").attr("src",response.url);
                        }
                    }
                    ...
                    if($('#gallery li').length == 0){
                        $('#gallery').html(response.html).fadeIn("fast");
