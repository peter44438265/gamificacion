...
(function($) {
	...
	var Mapplic = function() {
		...
		function Sidebar() {
			...
			this.addCategories = function(categories) {
				...
				$.each(categories, function(index, category) {
					...
					var link = $("<a data-rubro='"+category.title+"' id='v_markx' for='"+category.id+"'></a>").attr('href', '#').attr('title', category.title).css('background-color', category.color).text(category.title).click(function(e) {
			...
			this.addLocation = function(data) {
                            //alert(jQuery.makeArray(data))
				var item = $("<li id='v_tienda' data-id='"+data.category+"' for='"+data.tienda_id+"'></li>").addClass('mapplic-list-location').addClass('mapplic-list-shown');
		...
		var processData = function(data) {
			...
			self.levelselect = $('<select id="sl-malls" onchange="changemapplic()"></select>').addClass('mapplic-levels-select');
			...
			if (data.levels) {
				$.each(data.levels, function(index, value) {
					...
					//alert(value.locations)
					/* Iterate through locations */
                                        //alert($("#sl-malls").val());
                                        if(value.id=='11'){
                                                $.each(value.locations, function(index, valuel) {
                                                    //alert(valuel.mall+"-"+value.id)
                                                    if(valuel.mall==value.id){
                                                        var top = valuel.y * 100;
                                                        var left = valuel.x * 100;

                                                        if (valuel.pin != 'hidden') {
                                                                if (self.o.locations) {
                                                                        var target = '#' + valuel.id;
                                                                        if (valuel.action == 'redirect') target = valuel.link;

                                                                        var pin = $('<a></a>').attr('href', target).addClass('mapplic-pin').css({'top': top + '%', 'left': left + '%'}).appendTo(layer);
                                                                        pin.attr('data-location', valuel.id);
                                                                        pin.addClass(valuel.pin);
                                                                }
                                                        }

                                                        if (self.sidebar) self.sidebar.addLocation(valuel);
                                                    }
                                                });
                                            }
					nrlevels++;
