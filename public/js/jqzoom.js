	...
	$.fn.zoom = function(options){
		...
		var _option = {
			...
			thumb_image_height: 265,
                        	                 source_image_width:1680,  	// 放大图片的宽
			source_image_height: 1060,// 当前展示图片的高
			//source_image_width:950,  	// 放大图片的宽
			//source_image_height: 1800,	// 放大图片的高
			zoom_area_width: 600, 		// 放大图片的展示区域的宽
			zoom_area_height: 200,// 放大图片的展示区域的高
		...
		if($ul.is("ul") && $ul.children("li").length && $ul.find(".bzoom_big_image").length){
			...
			if(!$(".bzoom_small_thumbs").length){
				var top = _option.thumb_image_height+10,
					...
					//smwidth = (_option.thumb_image_width / _option.small_thumbs) - 10,
                                        smwidth = (_option.thumb_image_width / 5) - 10,
			...
			$li.add($bzoom_magnifier).mousemove(function(event){
				...
				$bzoom_magnifier.stop().animate({'opacity':1}, _option.speed*0.7, function(){
					$(this).hide();
				});
