ws_fade=function(options){
	...
	this.init=function(aCont){
		Images=$("img",aCont).get();$(Images).each(function(Index){
			if(!Index){
				$(this).show();
			}else{
				$(this).hide();
...
jQuery.fn.wowSlider=function(options){
	...
	options=$.extend({
		...
		},prev:"",next:"",duration:1000,delay:2000,outWidth:960,outHeight:360,width:960,height:360,caption:true,controls:true,autoPlay:true,bullets:true,onStep:function(){},stopOnHover:0},options);
		...
		var _this=$this.get(0);if(_this.addEventListener){
			_this.addEventListener("touchmove",function(e){
				if(isMoving){
					var dx=startX-e.touches[0].pageX;
					var dy=startY-e.touches[0].pageY;
					if(Math.abs(dx)>20||Math.abs(dy)>20){
						startX=startY=isMoving=0;forceGo(e,curIdx+(dx+dy>0?-1:1));
					}
				}
			},false);
			_this.addEventListener("touchstart",function(e){
				if(e.touches.length==1){
					startX=e.touches[0].pageX;startY=e.touches[0].pageY;isMoving=1;
				}else{isMoving=0;
			}
		},false);_this.addEventListener("touchend",function(e){
			isMoving=0;
		},false);
	}function go2(index){
		if(options.bullets){
	...
	}var autoPlayTimer;function restartPlay(){
		stopPlay();
		if(options.autoPlay){
	...
	if(c){
		c.css({
			position:"absolute",right:"2px",bottom:"2px",padding:"0 0 0 0"
		});
		ic.append(c);
	}
	if(c&&document.all){
...
if(d){
	d.css({
		position:"relative",
		display:"block",'background-color':"#E4EFEB",
		color:"#837F80",'font-family':"Lucida Grande,sans-serif",'font-size':"11px",'font-weight':"normal",'font-style':"normal",'-moz-border-radius':"5px",'border-radius':"5px",
		padding:"1px 5px",
		width:"auto",
		height:"auto",
		margin:"0 0 0 0",
...
function initBullets(){
	...
	if($thumbs.length){
		...
		var hideTime;$bullets_cont.hover(function(){
			if(hideTime){
...
//***********************************************
jQuery("#wowslider-container1").wowSlider({effect:"fade",prev:"Anterior",next:"Siguente",duration:10*100,delay:50*100,outWidth:800,outHeight:400,width:800,height:400,autoPlay:true,stopOnHover:false,loop:false,bullets:true,caption:true,controls:true});